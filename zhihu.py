...
ft = open('回答.txt','a',encoding='UTF-8')
...
for url in next_url:
    ...
    for answer in answers:
        count = count + 1
        ft.write("answer "+str(count) + ":\n" + answer + "\n")

    next_url.append(c["paging"]["next"])
    if c["paging"]["is_end"]:
        break
ft.write("total answers: "+str(count))
ft.close()